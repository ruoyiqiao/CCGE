...
python src/train.py \
    ...
    split='train' \
    cluster=0 \
    task=XArm7LeapHandCubeInBox \
    train=XArmLeapHandCubeInBoxPPO \
    task.env.enableContactSensors=False \
    reward_type=target+bonus+success+energy_reach+contact_coverage+reach \
    sim_device=cuda:0 \
    rl_device=cuda:0 \
    ...
    --cfg_train=XArmLeapHandCubeInBoxPPO \
    --seed=42 \
    --exp_name='PPO' \
    --logdir='xarm7_leap_cube_in_box' \
    --run_device_id=0 \
