...
python src/train.py \
    ...
    split='train' \
    cluster=0 \
    task=XArm7LeapHandFunctionalManipulationUnderarm \
    ...
    +task.env.objectNearEdge=True \
    +task.env.randomSceneXOnReset=True \
    +task.env.visDir=leap_singulation_ppo_curi_01_30_002 \
